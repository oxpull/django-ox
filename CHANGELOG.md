...
- `ox_prune` now rejects cutoffs on the first day of year one before the
  database binds them. With `USE_TZ=True` and a connection zone west of
  UTC, SQLite and MySQL overflow on those values even though cutoff
  subtraction succeeded. The command raises the existing out-of-range
  `CommandError` on every engine, including PostgreSQL.
