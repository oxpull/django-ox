...
This guide takes you from an empty Django project to background tasks
running in a worker, using Django's Tasks framework (`django.tasks`) with
django-ox as the backend. You will not install Redis, RabbitMQ or Celery.
The queue is a table in the database your project already has, and the
worker is a management command.
